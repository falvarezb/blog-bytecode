 ...
 */
case class ShortestPathInFiniteBoard(x: Int, y: Int, to: Coordinate) extends FiniteChessBoard with ShortestPath with BoardDim with DestinationVertex[Coordinate]

object ShortestPathInFiniteBoard{
  def apply(x: Int, y: Int, to: Coordinate): ShortestPathInFiniteBoard = new ShortestPathInFiniteBoard(x, y, to)
}
