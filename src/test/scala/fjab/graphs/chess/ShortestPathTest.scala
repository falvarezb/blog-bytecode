...
class ShortestPathTest extends FunSpec {
  ...
  describe("shortest path between (0,0) -> (3,3)"){
    ...
    it("in infinite board"){
      assert(ShortestPathInInfiniteBoard(to).findPath(List(List(from))) == solution)
    }
    it("in finite board of dimensions (4,4)"){
      val dim = (4,4)
      assert(ShortestPathInFiniteBoard(dim._1,dim._2, to).findPath(List(List(from))) == solution)
    }
    it("shared knowledge in infinite board"){
      assert(SKShortestPathInInfiniteBoard(to).findPath(List(List(from))) == solution)
    }
  }
  ...
  describe("shortest path between (0,0) -> (8,1)"){
    ...
    it("in finite board of dimensions (4,4)"){
      val dim = (4,4)
      assert(ShortestPathInFiniteBoard(dim._1, dim._2, to).findPath(List(List(from))) == List())
    }
    it("shared knowledge in infinite board"){
      assert(SKShortestPathInInfiniteBoard(to).findPath(List(List(from))) == solution)
    }
